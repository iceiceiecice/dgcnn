...
def load_data_partseg(partition):
    ...
    if partition == 'trainval':
        ...
        file = glob.glob(os.path.join(DATA_DIR, 'test', '*train*.h5')) \
               + glob.glob(os.path.join(DATA_DIR, 'test', '*val*.h5'))
    else:
        file = glob.glob(os.path.join(DATA_DIR, 'test', '*%s*.h5'%partition))
    for h5_name in file:
        f = h5py.File(h5_name, 'r+')
        # data = f['data'][:].astype('float32')
        # label = f['label'][:].astype('int64')
        ...
        data = f['data'][:].astype('float32')
        # label = f['label'][:].astype('int64')
        seg = f['label_seg'][:].astype('int64')
        label = np.zeros_like(seg).astype('int64')
        f.close()
        all_data.append(data)
        all_label.append(label)
        all_seg.append(seg)
