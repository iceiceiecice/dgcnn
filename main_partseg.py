...
def visualization(visu, visu_format, data, pred, seg, label, partseg_colors, class_choice):
    ...
    global visual_warning
    visu = visu.split('_')
    label = [0 for i in range(len(seg))]
    for i in range(0, data.shape[0]):
        RGB = []
        RGB_gt = []
        skip = False
        classname = class_choices[int(label[i])]
        # classname = class_choices[0]
        class_index = class_indexs[int(label[i])]
        # class_index = class_indexs[0]
        if visu[0] != 'all':
            if len(visu) != 1:
        ...
        if skip:
            ...
            filepath = 'outputs/' + args.exp_name + '/' + 'visualization' + '/' + classname + '/' + classname + '_' + str(
                class_index) + '_pred_' + '.' + visu_format
...
def train(args, io):
    ...
    if (len(train_dataset) < 100):
        ...
        drop_last = True
    train_loader = DataLoader(train_dataset, num_workers=0, batch_size=args.batch_size, shuffle=True, drop_last=drop_last)
